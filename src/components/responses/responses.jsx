...
const FarmerCard = ({ row, onViewDetails, onEdit, onDelete }) => (
  <Fade in timeout={300}>
    <Card 
      sx={{ 
        mb: 2, 
    ...
    >
      <CardContent>
        ...
        <Box sx={{ mt: 2, display: 'flex', justifyContent: 'flex-end', gap: 1 }}>
          <Tooltip title="Edit">
            <IconButton size="small" color="secondary" onClick={() => onEdit(row.person_id)}>
              <EditIcon />
            </IconButton>
          </Tooltip>
          ...
          <Tooltip title="Delete">
            <IconButton
              size="small"
              color="error"
              onClick={() => onDelete(row.person_id, row.form_data[0].farmer_name)}
            >
              <DeleteIcon />
            </IconButton>
          </Tooltip>
...
const FarmerRow = ({ row, onViewDetails, onEdit, onDelete }) => (
  <TableRow 
    hover 
    sx={{ 
      '&:hover': { 
  ...
  >
    ...
    <TableCell>
      <Chip
        label={<Typography variant="body2">{new Date(row.created_at).toLocaleDateString()}</Typography>}
        size="small"
        variant="outlined"
        sx={{ fontWeight: 600 }}
      />
...
const FarmerTable = ({ data, rowsPerPage, page, onViewDetails, onEdit, onDelete }) => {
  const theme = useTheme();
  const isMobile = useMediaQuery(theme.breakpoints.down('md'));
...
const DeleteConfirmDialog = ({ open, onClose, onConfirm, farmerName, loading }) => (
  <Dialog open={open} onClose={onClose} maxWidth="xs" fullWidth>
    ...
    <DialogContent>
      <DialogContentText>
        Are you sure you want to <strong>permanently delete</strong> the record for
...
const EnhancedFarmerResponseTable = () => {
  ...
  const handleEdit = (person_id) => {
    navigate(`/surveyform?id=${person_id}`);
  };
  ...
  // Delete handlers
  const handleDeleteClick = (personId, farmerName) => {
    setDeleteDialog({ open: true, personId, farmerName });
  };

  const handleDeleteCancel = () => {
    setDeleteDialog((s) => ({ ...s, open: false }));
  };

  const handleDeleteConfirm = async () => {
    const { personId } = deleteDialog;
    setLoading(true);
    try {
      await axios.post(
        `${import.meta.env.VITE_API_URL}/floods/api/farmer/delete/`,
      ...
      setError(
        err.response?.data?.error ||
          'Failed to delete farmer record. Please try again.'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <Container maxWidth="xl" sx={{ py: 3 }}>
      ...
      <DeleteConfirmDialog
        ...
        farmerName={deleteDialog.farmerName}
        loading={loading}
      />
    </Container>
  );
