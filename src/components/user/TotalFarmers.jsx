...
export default function TotalFarmers() {
  ...
  const fetchFarmerData = async (pageNum = 1, search = "") => {
    ...
    try {
      ...
      const transformedData = results.map((item) => ({
        ...
        name: item.form_data[0]?.farmer_name || t('floodDashboard.totalFarmers.no_results'),
        location:
          item.village_data[0]?.tehsil_data[0]?.city_data[0]?.name || t('floodDashboard.totalFarmers.no_results'),
        tehsil: item.village_data[0]?.tehsil_data[0]?.name || t('floodDashboard.totalFarmers.no_results'),
        village: item.village_data[0]?.display_name || t('floodDashboard.totalFarmers.no_results'),
        landSize: item.form_data[0]?.totalLandOwned
          ? `${item.form_data[0].totalLandOwned}`
          : t('floodDashboard.totalFarmers.no_results'),
        crop: item.form_data[0]?.cropsPlanted || t('floodDashboard.totalFarmers.no_results'),
        phone: item.form_data[0]?.mobileNumber || t('floodDashboard.totalFarmers.no_results'),
        houseStatus: item.form_data[0]?.houseStatus
          ? t(`floodDashboard.totalFarmers.table_headers.status_${item.form_data[0].houseStatus.toLowerCase().replace(' ', '_')}`)
          : t('floodDashboard.totalFarmers.table_headers.status_no_damage'),
      }));
      ...
      alert(t('floodDashboard.errors.unable_to_load_farmers'));
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleGeneratePDF = async () => {
    ...
    try {
      ...
      if (!pdfUrl) {
        throw new Error(t('floodDashboard.totalFarmers.pdf_error'));
      }
      ...
      alert(t('floodDashboard.totalFarmers.pdf_error'));
    } finally {
      setPdfLoading(false);
    }
  };
      ...
      <Box sx={{ minHeight: "100vh", backgroundColor: "#121212" }}>
        ...
        <Box sx={{ p: { xs: 2, sm: 3, md: 4 } }}>
          ...
          <Box sx={{ mb: 4 }}>
            ...
            >
              {t('floodDashboard.totalFarmers.title')}
            </Typography>
            <Typography
              variant="body2"
              sx={{
            ...
            >
              {t('floodDashboard.totalFarmers.subtitle')}
            </Typography>
              ...
              >
                ...
                <Box sx={{ flex: 1 }}>
                  <Typography variant="body2" sx={{ color: "#b0b0b0", mb: 1 }}>
                    {t('floodDashboard.totalFarmers.active_filters')}
                  </Typography>
                  <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                    ...
                    {!selectedState &&
                      ...
                      !selectedVillage && (
                        ...
                        >
                          {t('floodDashboard.totalFarmers.no_filters')}
                        </Typography>
                      )}
                  </Stack>
                </Box>
          ...
          >
            <CardContent>
              ...
              >
                {t('floodDashboard.totalFarmers.filter_options')}
              </Typography>
              <Box
                sx={{
                  display: "grid",
              ...
              >
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t('floodDashboard.totalFarmers.select_state')}
                      variant="outlined"
                      size="small"
                      sx={{
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t('floodDashboard.totalFarmers.select_district')}
                      variant="outlined"
                      size="small"
                      sx={{
                        "& .MuiOutlinedInput-root": {
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t('floodDashboard.totalFarmers.select_tehsil')}
                      variant="outlined"
                      size="small"
                      sx={{
                        "& .MuiOutlinedInput-root": {
                ...
                <Autocomplete
                  ...
                  renderInput={(params) => (
                    <TextField
                      {...params}
                      label={t('floodDashboard.totalFarmers.select_village')}
                      variant="outlined"
                      size="small"
                      sx={{
                        "& .MuiOutlinedInput-root": {
              ...
              >
                <TextField
                  fullWidth
                  label={t('floodDashboard.totalFarmers.search_placeholder')}
                  variant="outlined"
                  size="small"
                  value={searchTerm}
                  ...
                  placeholder={t('floodDashboard.totalFarmers.search_placeholder')}
                />
                <Button
                  variant="contained"
                ...
                >
                  {pdfLoading ? t('floodDashboard.totalFarmers.generating_pdf') : t('floodDashboard.totalFarmers.generate_pdf')}
                </Button>
              </Box>
            </CardContent>
          </Card>
              ...
              <Table sx={{ minWidth: 650 }}>
                <TableHead>
                  <TableRow>
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.image')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.farmer_details')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.location')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.land_size')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.crop')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.contact')}
                    </TableCell>
                    <TableCell
                      sx={{
                        color: "#90caf9",
                    ...
                    >
                      {t('floodDashboard.totalFarmers.table_headers.status')}
                    </TableCell>
                  </TableRow>
                </TableHead>
                <TableBody>
                    ...
                    <TableRow>
                      ...
                      >
                        ...
                        <Typography sx={{ mt: 2, color: "#b0b0b0" }}>
                          {t('floodDashboard.totalFarmers.loading_data')}
                        </Typography>
                      </TableCell>
                    </TableRow>
                  ) : farmers.length === 0 ? (
                      ...
                      >
                        ...
                        >
                          {farmer.image ? (
                            ...
                            >
                              {farmer.name?.charAt(0).toUpperCase() || t('floodDashboard.totalFarmers.no_results')}
                            </Box>
                          )}
                        </TableCell>
                        <TableCell
                        ...
                        >
                          <Chip
                            ...
                            sx={{
                              backgroundColor:
                                farmer.houseStatus === t('floodDashboard.totalFarmers.table_headers.status_no_damage')
                                  ? "#2e7d32"
                                  : farmer.houseStatus === t('floodDashboard.totalFarmers.table_headers.status_partially_damaged')
                                  ? "#f57c00"
                                  : "#d32f2f",
                              color: "#ffffff",
                              fontWeight: 600,
        ...
        >
          <Box sx={{ textAlign: "center" }}>
            ...
            <Typography sx={{ mt: 2, fontSize: "1.2rem", fontWeight: 500 }}>
              {t('floodDashboard.totalFarmers.pdf_generating')}
            </Typography>
            <Typography sx={{ mt: 1, fontSize: "0.9rem", color: "#b0b0b0" }}>
              {t('floodDashboard.totalFarmers.pdf_wait')}
            </Typography>
          </Box>
        </Backdrop>
      </Box>
