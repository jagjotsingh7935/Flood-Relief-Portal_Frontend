...
const FloodDashboardMap = () => {
  ...
  const [statsData, setStatsData] = useState([
    {
      title: t('floodDashboard.stats.affected_population'),
      value: "-",
      change: "-",
      color: "#ff4081",
    ...
    {
      title: t('floodDashboard.stats.rescue_operations'),
      value: "-",
      change: "-",
      color: "#69f0ae",
      icon: "mdi:lifebuoy",
    ...
    {
      title: t('floodDashboard.stats.houses_partially_damaged'),
      value: "-",
      change: "-",
      color: "#ffb74d",
      icon: "mdi:home-alert",
    ...
    {
      title: t('floodDashboard.stats.houses_fully_damaged'),
      value: "-",
      change: "-",
      color: "#ff5252",
    ...
    {
      title: t('floodDashboard.stats.land_owned_affected'),
      value: "- / -",
      change: "-",
      color: "#00d4ff",
      icon: "mdi:land-plots",
    ...
    {
      title: t('floodDashboard.stats.estimated_crop_loss'),
      value: "-",
      change: "-",
      color: "#ff4081",
  ...
  const fetchStats = async () => {
    try {
      const res = await axios.get(`${import.meta.env.VITE_API_URL}/dataAddition/api/top/card/stats/`);
      setStatsres(res?.data);
    } catch (error) {
  ...
  const statisticsData = useMemo(
    () => [
      {
        title: t('floodDashboard.stats.total_farmers'),
        value: statsres ? `${statsres.total_farmer_count}` : "-",
        icon: "mdi:farmer",
        color: "#00d4ff",
        change: "+12%",
        link: '/totalfarmers'
      ...
      {
        title: t('floodDashboard.stats.affected_areas'),
        value: statsres ? `${statsres.affected_areas_count}` : "-",
        icon: "mdi:map-marker-radius",
        color: "#ff5252",
        change: "+5",
        link: '/affectedareasstats'
      },
      {
        title: t('floodDashboard.stats.total_amount_needed'),
        value: statsres ? `${statsres.total_amount_needed}` : "-",
      ...
      {
        title: t('floodDashboard.stats.total_amount_received'),
        value: statsres ? `${statsres.total_amount_received}` : "-",
        icon: "mdi:clock",
        color: "#ffb74d",
        change: "-3%",
        link: null
      ...
      {
        title: t('floodDashboard.stats.total_land_affected'),
        value: statsres ? `${statsres.total_land_affected}` : "-",
        icon: "mdi:land-plots",
        color: "#ff4081",
        change: "+15%",
        link: null
      ...
      {
        title: t('floodDashboard.stats.relief_camps'),
        value: statsres ? `${statsres.relief_camps}` : "-",
        icon: "mdi:home-group",
        color: "#9c27b0",
        change: "+2",
        link: null
  ...
  const fetchStatsData = useCallback(async (query = "") => {
    try {
      ...
      // setPlacesError(t('floodDashboard.errors.unable_to_load_stats'));
    }
  }, [t]);
  ...
  const fetchFarmersData = useCallback(
    async (query = "", status = "All", cropLoss = "All") => {
      ...
      try {
        ...
        setPlacesError(t('floodDashboard.errors.unable_to_load_farmers'));
        setFarmersData([]);
        setCount(0);
      } finally {
    ...
    [currentPage, rowsPerPage, searchQuery, t]
  );
  ...
  const handleVillageSelect = useCallback((villageData) => {
    ...
    setSelectedVillageId(villageData?.villageData?.id);
    if (!villageData) {
    ...
    if (mapInstanceRef.current) {
      ...
      placeMarkerRef.current = L.marker([lat, lng], {
        ...
        .bindPopup(
          ...
          <div style="background: #1a1a1a; color: white; border-radius: 8px; padding: 12px; max-width: 250px;">
            ...
            <p style="margin: 4px 0;"><strong>${t('floodDashboard.villages.risk')}:</strong> Village</p>
      ...
      setSnackbarMessage(t('floodDashboard.snackbar.zoomed_to', { village: villageName }));
      setSnackbarOpen(true);
    }
  }, [handlePlaceDragEnd, t]);
  ...
  const fetchSuggestions = useCallback(
    async (query) => {
      ...
      try {
        ...
        setPlacesError(t('floodDashboard.errors.unable_to_load_suggestions'));
  ...
  useEffect(() => {
    const fetchVillageData = async () => {
      try {
        ...
        setPlacesError(t('floodDashboard.errors.unable_to_load_villages'));
      }
    };
    fetchVillageData();
  }, [t]);
  ...
  }, [searchQuery, fetchSuggestions, t]);
      ...
      <ThemeProvider theme={darkTheme}>
        ...
        <Box sx={{ height: "auto", display: "flex", flexDirection: "column" }}>
          ...
          <SearchSection
            ...
            placeholder={t('floodDashboard.search_placeholder')}
                    ...
                    >
                      <Box sx={{ flex: 1, minHeight: isMobile ? "400px" : "700px", minWidth: "70vw" }} id="map-section">
                        <MapSection
                          ...
                          onSnackbarMessage={(message) => setSnackbarMessage(message)}
                          isMobile={isMobile}
                        />
                      </Box>
                    </CardContent>
