...
const ExcelUploadDownload = () => {
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      // Auto-download errors Excel if present
    if (response.data.error_file_url) {
      const link = document.createElement('a');
      link.href = response.data.error_file_url;
      link.setAttribute('download', '');
      link.setAttribute('target', '_blank');
      document.body.appendChild(link);
      link.click();
      document.body.removeChild(link);
      setError(`Processed with ${response.data.errors_count} errors. Error file downloaded.`);
    }
      setFile(null);
    } catch (err) {
