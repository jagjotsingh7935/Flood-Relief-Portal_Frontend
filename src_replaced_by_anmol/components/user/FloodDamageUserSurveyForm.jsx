...
const FloodDamageUserSurveyForm = () => {
  ...
  const handleWebcamCapture = () => {
    ...
    if (imageSrc) {
      fetch(imageSrc)
        .then((res) => res.blob())
        .then((blob) => {
          const file = new File([blob], `${currentImageField}_${Date.now()}.jpg`, { type: "image/jpeg" });
          setFormData({
            ...formData,
  ...
  const validateStep = (step) => {
    ...
    switch (step) {
      case 0:
        ...
        if (!selectedState) newErrors.stateName = "State is required";
        if (!selectedDistrict) newErrors.district = "District is required";
        if (!selectedTehsil) newErrors.tehsil = "Tehsil is required";
        ...
        break;
      case 1:
        if (!formData.totalLandOwned)
          newErrors.totalLandOwned = "Total land owned is required";
        if (!formData.landAffected)
          newErrors.landAffected = "Land affected is required";
        break;
      case 4:
        if (!formData.surveyorName)
        ...
        if (!formData.verifiedBy)
          newErrors.verifiedBy = "Verification image is required";
        if (!formData.farmerImage)
          newErrors.farmerImage = "Farmer image is required";
        break;
    }
    ...
    if (name === "pincode" && value) {
      ...
      try {
        const response = await fetch(
          `https://nominatim.openstreetmap.org/search?format=json&postalcode=${value}&limit=1`,
          {
            headers: {
              "User-Agent": "FloodDamageSurveyApp/1.0 (your.email@example.com)",
            },
          }
        );
        const data = await response.json();
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `${import.meta.env.VITE_API_URL}/floods/api/farmer/data/add/user/`,
        {
          method: "POST",
          body: formDataToSend,
        }
      ...
      if (response.ok) {
        ...
        setSnackbar({
          open: true,
          message: `Survey submitted successfully!,${response.statusText}`,
          severity: "success",
        });
      ...
      setSnackbar({
        open: true,
        message: `Failed to submit survey, getting error-${error.message}`,
        severity: "error",
      });
    } finally {
      setIsSubmitting(false);
  ...
  const renderRadioField = (label, name, options, xs = 12, md = 6) => (
    <Grid item xs={xs} md={md} key={name}>
      <FormControl fullWidth>
        ...
        <RadioGroup name={name} value={formData[name]} onChange={handleChange}>
          {options.map((option) => (
            <FormControlLabel
              key={option}
              value={option}
    ...
    switch (step) {
      case 0:
        return renderFormSection("Personal Information", "mdi:account", [
          <Grid item xs={12} md={6} key="farmerName">
            <TextField
          ...
          <Grid item xs={12} md={6} key="fatherName">
            <TextField
              label="Father's Name"
              name="fatherName"
          ...
          <Grid item xs={12} md={6} key="mobileNumber">
            <TextField
              label="Mobile Number"
              name="mobileNumber"
          ...
          <Grid item xs={12} md={6} key="pincode">
            <TextField
              label="Pincode"
              name="pincode"
          ...
          // <Grid item xs={12} md={6} key="cityName">
          //   <TextField
          //     label="City Name"
          //     name="cityName"
          ...
          renderRadioField(
            "House Status",
            "houseStatus",
            ["Safe", "Partially Damaged", "Fully Damaged"],
            12,
            12
          ),
        ]);
      ...
      case 1:
        return renderFormSection("Land & Crop Information", "mdi:sprout", [
          <Grid item xs={12} md={6} key="totalLandOwned">
            <TextField
          ...
          <Grid item xs={12} md={6} key="landAffected">
            <TextField
          ...
          <Grid item xs={12} md={6} key="cropsPlanted">
            <TextField
          ...
          <Grid item xs={12} md={6} key="cropsLost">
            <TextField
      ...
      case 2:
        return renderFormSection(
          ...
          [
            ...
            renderRadioField(
              "Manure/Organic Fertilizer Needed",
              "manureFertilizer",
              [
                "Not Required",
                "Required (Basic)",
      ...
      case 3:
        return renderFormSection(
          ...
          [
            renderRadioField(
              "Livestock Damage / Fodder Requirement",
              "livestockDamage",
              [
                "No Damage",
                "Minor Loss/Damage",
            ...
            renderRadioField(
              "Housing Repair/Reconstruction Requirement",
              "housingRepair",
              [
                "No Repair Needed",
                "Minor Repairs",
      ...
      case 4:
        return renderFormSection(
          ...
          "mdi:clipboard-check",
          [
            <Grid item xs={12} md={6} key="surveyorName">
              <TextField
                label="Surveyor Name"
                name="surveyorName"
            ...
            <Grid item xs={12} md={6} key="surveyDate">
              <TextField
                label="Survey Date"
                name="surveyDate"
            ...
            <Grid item xs={12} md={6} key="verifiedBy">
              <TextField
            ...
            <Grid item xs={12} md={6} key="farmerImage">
              <TextField
                label="Farmer Image"
                name="farmerImage"
                type="file"
  ...
  const progress =
    activeStep < steps.length ? ((activeStep + 1) / steps.length) * 100 : 100;
  ...
  return (
    ...
    >
        ...
        <Box sx={{ textAlign: "center", mb: { xs: 2, sm: 3, md: 4 } }}>
          ...
          >
            ...
            >
              ...
              <strong>Mera Pind Research and Welfare Society Punjab</strong> to
              collect authentic data of individual farmers/landowners affected
              by flooding.
            </Typography>
          </Alert>
        </Box>
       ...
       <Stepper 
        ...
        {activeStep === steps.length ? (
         <Card 
            ...
            >
              All steps have been completed. Please review your information and
              submit the form.
            </Typography>
            <Button
      ...
      <Dialog open={webcamOpen} onClose={closeWebcam} maxWidth="sm" fullWidth>
        <DialogTitle>Capture Image with Webcam</DialogTitle>
        <DialogContent>
          <Webcam
            audio={false}
            ref={webcamRef}
        ...
        <DialogActions>
          <Button onClick={closeWebcam}>Cancel</Button>
          <Button onClick={handleWebcamCapture} variant="contained">
            Capture
          </Button>
        </DialogActions>
      </Dialog>
...
export default FloodDamageUserSurveyForm;